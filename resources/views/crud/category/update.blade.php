...
<div class="p-8">
    <header class="bg-neutral-800 shadow-sm rounded-xl p-4 mb-8 flex justify-between items-center">
        <div>
            <h2 class="text-2xl font-bold text-neutral-100">Update Category</h2>
            <p class="text-neutral-400">Edit and update category details</p>
        </div>
    ...
    <form id="categoryForm" class="w-full" action="{{ route('update-category', $category->id) }}" method="POST">
        @csrf
        @method('PUT')
        <!-- Basic Information -->
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <textarea name="description" rows="3" required
                              class="w-full bg-neutral-700 border border-neutral-600 rounded-lg px-4 py-2 text-neutral-100 focus:outline-none focus:border-red-500"
                              placeholder="Brief description of the category">{{ $category->description }}</textarea>
                </div>
            </div>
        </div>
    </form>
