...
@section('content')
    ...
    <style>
        .scrollable-section {
            ...
            scrollbar-color: rgba(239, 68, 68, 0.5) rgba(38, 38, 38, 0.5);
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            const scrollContainers = document.querySelectorAll('.scroll-btn-container');
            
            scrollContainers.forEach(container => {
                const scrollSection = container.querySelector('.scrollable-section');
                const prevBtn = container.querySelector('.scroll-btn.prev');
                const nextBtn = container.querySelector('.scroll-btn.next');
                
                if (prevBtn && nextBtn && scrollSection) {
                    // Initially hide prev button
                    prevBtn.style.display = 'none';
                    
                    // Check if scrolling is needed
                    if (scrollSection.scrollWidth <= scrollSection.clientWidth) {
                    ...
                    scrollSection.addEventListener('scroll', () => {
                        const isAtStart = scrollSection.scrollLeft === 0;
                        const isAtEnd = scrollSection.scrollLeft + scrollSection.clientWidth >= scrollSection.scrollWidth - 10;
